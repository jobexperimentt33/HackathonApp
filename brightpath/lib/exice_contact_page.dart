...
class _ExciseContactPageState extends State<ExciseContactPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Excise Department Directory'),
        ...
        child: SingleChildScrollView(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                _buildMottoBanner(context),
                const SizedBox(height: 24),
                _buildRegionSelector(),
                const SizedBox(height: 20),
                if (selectedRegion != null) _buildDistrictSelector(),
                const SizedBox(height: 20),
                if (selectedDistrict != null) _buildContactList(),
              ],
            ),
          ),
        ),
  ...
  Widget _buildMottoBanner(BuildContext context) {
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(
            'Prudent in actions and dedicated to the betterment of the country',
            style: TextStyle(
              fontSize: 14,
              color: Colors.white.withOpacity(0.9),
            ),
        ...
        Container(
          ...
          child: DropdownButtonHideUnderline(
            child: DropdownButton<String>(
              ...
              hint: const Padding(
                padding: EdgeInsets.symmetric(horizontal: 16.0),
                child: Text('Choose a district'),
              ),
              isExpanded: true,
              items: regions[selectedRegion]!.map((String district) {
                return DropdownMenuItem<String>(
                  value: district,
                  child: Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 16.0),
  ...
  Widget _buildContactList() {
    final contacts = contactDetails[selectedDistrict] ?? [];
        ...
        child: Column(
          ...
          children: [
            ...
            if (contact['phone']?.isNotEmpty ?? false)
              _buildContactDetail(Icons.phone, contact['phone']!, 'tel:${contact['phone']}'),
            if (contact['mobile']?.isNotEmpty ?? false)
              _buildContactDetail(Icons.smartphone, contact['mobile']!, 'tel:${contact['mobile']}'),
            if (contact['email']?.isNotEmpty ?? false)
              _buildContactDetail(Icons.email, contact['email']!, 'mailto:${contact['email']}'),
            if (contact['address']?.isNotEmpty ?? false)
              _buildContactDetail(Icons.location_on, contact['address']!, null),
          ],
        ),
      ),
    );
  ...
  Widget _buildContactDetail(IconData icon, String text, String? url) {
    Widget content = Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
